...
![그림 5.12: 5장 의사결정 흐름도 — 신뢰구간 vs 가설검정의 선택과 SE 계산의 차이](figures/fig_5_12_decision_tree.png)

연구 질문이 "**어떤 값이 그럴듯한가**"라면 신뢰구간을 사용하여 모수의 그럴듯한 값들의 범위를 제시하고, SE 계산에는 표본비율 $\hat{p}$를 대입한다. 반면 "**이 특정 값이 그럴듯한가**"를 묻는다면 가설검정을 사용하며, $H_0$가 참이라는 가정 하에서 분석하므로 SE에 귀무값 $p_0$를 대입한다. 두 접근법 모두 동일한 표본추출 분포 위에서 작동하지만 SE 공식에 들어가는 비율이 다르다는 점이 핵심이다.

검정 결과의 표현에도 주의가 필요하다. $p$-값이 $\alpha$ 미만이면 $H_0$를 **기각**하지만, 그 반대 경우는 "$H_0$ 기각 못함"이라 표현하지 "$H_0$ 채택"이라 하지 않는다. 자료가 $H_0$를 강하게 부인하지 못한 것일 뿐, $H_0$가 참임이 증명된 것은 아니기 때문이다.

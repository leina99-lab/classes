...
기저 단계. $|V_1|=1$이면 $V_1=\{v_0\}$이다. 조건에서 $|N(\{v_0\})|\ge 1$이므로 $v_0$은 적어도 한 이웃 $w$를 가진다. 모서리 하나 $\{v_0,w\}$가 곧 완전 매칭이다.

귀납 단계. $|V_1|=k\ge 2$라 하고, $V_1$의 크기가 $k$보다 작은 모든 이분 그래프에 대해 (홀의 조건이 성립하면 완전 매칭이 존재한다는) 명제가 참이라고 가정한다. 두 경우로 나눈다.

**경우 1.** $V_1$의 공집합이 아닌 모든 진부분집합 $A$에 대해 강한 부등식 $|N(A)|\ge|A|+1$이 성립하는 경우. $V_1$에서 꼭지점 $v$ 하나를 고르고, 그 이웃 $w\in N(\{v\})$ 하나를 고른다($|N(\{v\})|\ge 1$이므로 존재한다). 이제 $v$와 $w$를 짝짓고, 두 꼭지점을 각각 제거한 이분 그래프 $G'$를 본다. 여기서 $V_1'=V_1\setminus\{v\}$이다. $G'$에서 홀의 조건이 여전히 성립함을 확인한다. 먼저 $A=\emptyset$이면 $|N_{G'}(\emptyset)|=0\ge 0=|\emptyset|$으로 조건이 자명하게 성립한다. 공집합이 아닌 $A\subseteq V_1'$에 대해서는, $G'$에서의 이웃이 $G$에서의 이웃에서 많아야 $w$ 하나만 사라진 것이므로

$$|N_{G'}(A)|\ge|N(A)|-1.$$

그런데 이 $A$는 공집합이 아닌 $V_1$의 진부분집합이므로 경우 1의 가정에서 $|N(A)|\ge|A|+1$이고, 따라서 $|N_{G'}(A)|\ge|A|$가 된다. 즉 $G'$도 홀의 조건을 만족한다. $G'$는 $|V_1'|=k-1<k$이므로 귀납 가정에 의해 완전 매칭 $M'$을 가진다. 여기에 모서리 $\{v,w\}$를 더한 $M'\cup\{\{v,w\}\}$가 $G$의 완전 매칭이다.

**경우 2.** 어떤 공집합이 아닌 진부분집합 $A_0\subsetneq V_1$이 등호 $|N(A_0)|=|A_0|$를 만족하는 경우. 두 조각으로 나누어 각각 매칭한 뒤 합친다.

먼저 $A_0$과 그 이웃 $N(A_0)$만으로 이루어진 이분 그래프 $G_1$을 본다. $A_0$의 임의의 부분집합 $A$에 대해 그 이웃은 $N(A_0)$ 안에 들어가므로 $G_1$에서의 이웃과 $G$에서의 이웃이 같고, 가정에서 $|N(A)|\ge|A|$이다. 그러므로 $G_1$도 홀의 조건을 만족한다. $|A_0|<k$이므로 귀납 가정에 의해 $G_1$은 $A_0$ 전체를 $N(A_0)$로 보내는 완전 매칭 $M_1$을 가진다. 등호 $|N(A_0)|=|A_0|$ 때문에 $M_1$은 $N(A_0)$의 꼭지점을 남김없이 모두 쓴다.

다음으로 나머지 부분, 곧 $V_1\setminus A_0$과 $V_2\setminus N(A_0)$로 이루어진 이분 그래프 $G_2$를 본다. $G_2$에서도 홀의 조건이 성립함을 보인다. 임의의 $B\subseteq V_1\setminus A_0$을 잡고, $G$에서 $A_0\cup B$의 이웃을 본다. $A_0$과 $B$는 서로소이므로

$$|N(A_0\cup B)|\ge|A_0\cup B|=|A_0|+|B|.$$

한편 $N(A_0\cup B)=N(A_0)\cup N(B)$이고, $G_2$에서의 $B$의 이웃은 정의상 $N_{G_2}(B)=N(B)\setminus N(A_0)$, 곧 $B$의 이웃 중 $N(A_0)$에 아직 들지 않은 것들이다. 그러므로 $N(A_0\cup B)$를 $N(A_0)$ 부분과 그 바깥 부분으로 갈라 세면 $|N(A_0\cup B)|=|N(A_0)|+|N_{G_2}(B)|$이다. 이를 위 부등식에 넣으면

$$|N(A_0)|+|N_{G_2}(B)|\ge|A_0|+|B|$$

이고, $|N(A_0)|=|A_0|$를 소거하면 $|N_{G_2}(B)|\ge|B|$를 얻는다. 따라서 $G_2$도 홀의 조건을 만족한다. $A_0$이 공집합이 아니므로 $|V_1\setminus A_0|=k-|A_0|<k$이고, 귀납 가정에 의해 $G_2$는 $V_1\setminus A_0$ 전체를 매칭하는 완전 매칭 $M_2$를 가진다.

이제 $M_1$은 $A_0$을 $N(A_0)$로, $M_2$는 $V_1\setminus A_0$을 $V_2\setminus N(A_0)$로 보내며 두 짝짓기가 쓰는 꼭지점이 서로 겹치지 않는다. 그러므로 $M_1\cup M_2$가 $V_1$ 전체를 매칭하는 완전 매칭이다.

![fig_hall](figs/fig_hall.png)

위 그림이 경우 2의 분할을 나타낸다. 등호인 $A_0$을 기준으로 그래프가 두 조각으로 갈린다. 왼쪽 조각 $G_1$은 $A_0$과 그 이웃 $N(A_0)$만으로 이루어지고, 오른쪽 조각 $G_2$는 나머지 $V_1\setminus A_0$과 $V_2\setminus N(A_0)$로 이루어진다. $G_1$은 $N(A_0)$ 안에서만, $G_2$는 그 바깥에서만 짝을 찾으므로 두 매칭이 쓰는 꼭지점이 겹칠 수 없고, 그래서 $M_1$과 $M_2$를 그냥 합치기만 해도 $V_1$ 전체의 완전 매칭이 된다.

두 경우 모두에서 완전 매칭을 구성하였으므로, 귀납법에 의해 홀의 조건이 성립하는 모든 이분 그래프는 완전 매칭을 가진다. $\blacksquare$

이 증명에서 약한 귀납법이 아니라 강한 귀납법이 필요한 까닭은 경우 2에 있다. 거기서는 크기가 $|A_0|$와 $k-|A_0|$인 두 이분 그래프에 귀납 가정을 적용하는데, 이 두 크기는 바로 앞 단계인 $k-1$로 고정되지 않고 $1$ 이상 $k-1$ 이하의 어떤 값도 될 수 있다. 따라서 바로 앞 단계만 가정하는 약한 귀납법으로는 부족하고, $k$보다 작은 모든 크기에 대해 가정하는 강한 귀납법이 필요하다.

...
## 1.12.4 군(group) — 가장 단순한 대수적 구조

§1.12.3 에서 우리는 ℤ_m 위에 여섯 가지 대수적 성질이 성립함을 보았다. 이 여섯 성질 중 *덧셈에 관한 네 가지*만 따로 묶으면, ‘되돌릴 수 있는 연산을 가진 집합' 이라는 이름이 붙는 가장 기본적인 대수 구조가 된다. 그 이름이 *군*(group)이다.

### 1.12.4.1 군의 형식적 정의

> **정의 (군)** — 집합 `G`와 이항연산 `* : G × G → G`의 쌍 `(G, *)` 가 다음 *네 공리*를 모두 만족할 때, 이 쌍을 **군**(group)이라 한다.
>
> **(G1) 폐쇄성**(closure) — 임의의 `a, b ∈ G` 에 대해 `a * b ∈ G`.
> **(G2) 결합성**(associativity) — 임의의 `a, b, c ∈ G` 에 대해 `(a * b) * c = a * (b * c)`.
> **(G3) 항등원**(identity element) — 임의의 `a ∈ G` 에 대해 `e * a = a * e = a` 를 만족하는 원소 `e ∈ G`가 존재한다.
> **(G4) 역원**(inverse element) — 각 `a ∈ G` 에 대해 `a * a⁻¹ = a⁻¹ * a = e` 를 만족하는 원소 `a⁻¹ ∈ G`가 존재한다.
>
> 추가로 **(G5) 교환성**(commutativity) — 임의의 `a, b ∈ G` 에 대해 `a * b = b * a` — 까지 성립하면, 그 군을 **교환군**(abelian group, commutative group)이라 한다.

여기서 *폐쇄성* 은 ‘연산이 *G* 밖으로 빠져나가지 않는다' 는 보장이고, *결합성* 은 ‘괄호를 어떻게 쳐도 결과가 같다' 는 보장이며, *항등원* 은 ‘아무 일도 하지 않는 특별한 원소' 의 존재이고, *역원* 은 ‘이 연산을 *되돌릴 수 있다*' 는 보장이다.

### 1.12.4.2 직관 — 군은 ‘되돌릴 수 있는 연산'

군의 본질을 한 문장으로 요약하면 다음과 같다.

> *군이란 연산을 적용하면 언제든 원위치로 돌아올 수 있는 집합이다.*

‘되돌릴 수 있다' 는 표현이 곧 (G4) 역원 공리이다. 시계의 시침을 시계 방향으로 5칸 돌렸으면 반시계 방향으로 5칸 돌려 원위치로 갈 수 있다. 한 정수 `a` 에 다른 정수 `b` 를 더했으면 `b` 를 다시 빼서 원래 자리로 돌아올 수 있다. 이런 ‘가역성'(invertibility) 이 군 구조의 심장이다.

### 1.12.4.3 친숙한 군의 예

군이라는 추상 개념이 처음 보면 낯설지만, 사실 우리는 이미 군을 여러 번 만나 왔다.

| 집합 | 연산 | 항등원 | 역원 | 군인가? | 교환군인가? |
|---|---|---|---|---|---|
| ℤ (정수) | `+` | `0` | `-a` | 예 | 예 |
| ℚ (유리수) | `+` | `0` | `-a` | 예 | 예 |
| ℝ (실수) | `+` | `0` | `-a` | 예 | 예 |
...
| ℝ \ {0} | `×` | `1` | `1/a` | 예 | 예 |
| ℤ | `×` | `1` | `1/a` (정수 아님) | *아니다* | — |
| ℕ (자연수) | `+` | `0` | `-a` (자연수 아님) | *아니다* | — |
| `n × n` 가역 실행렬 `GL(n, ℝ)` | 행렬곱 | `Iₙ` | `A⁻¹` | 예 | *아니다* (`n ≥ 2`) |

표 마지막 두 행은 *반례*로서 특히 중요하다. ℤ 위에서 곱셈은 (G1)~(G3) 까지 모두 만족하지만 (G4) 역원이 깨진다. `2` 의 곱셈에 대한 역원은 `1/2` 인데 이는 정수가 아니다. 즉 *ℤ 안에서는 곱셈을 되돌릴 수 없다*. 그래서 `(ℤ, ×)` 는 군이 아니다. 비슷한 이유로 `(ℕ, +)` 도 군이 아니다 — `2 + (-2) = 0` 인데 `-2` 가 자연수가 아니다.

행렬군 `GL(n, ℝ)` 은 군이지만 *교환군이 아닌* 가장 흔한 예이다. 일반적으로 `AB ≠ BA` 이다. 이 비가환성이 양자역학·로보틱스·컴퓨터그래픽스에서 결정적 역할을 한다.

### 1.12.4.4 (ℤ_m, +_m) 이 *교환군이 되는 이유* — 네 공리의 검증

이제 본 절의 핵심이다. §1.12.3 의 여섯 성질 중 *덧셈에 관한 네 가지* 가 정확히 군의 네 공리에 대응함을 확인한다.

| 군 공리 | ℤ_m 의 대응 사실 | 출처 |
|---|---|---|
| (G1) 폐쇄성 | `a, b ∈ ℤ_m ⟹ (a +_m b) ∈ ℤ_m` | `+_m` 의 정의 자체에 의해 결과가 `0 ≤ r < m` 범위 |
| (G2) 결합성 | `(a +_m b) +_m c = a +_m (b +_m c)` | 정리 5 (정수 덧셈의 결합성을 mod 산술로 옮긴 것) |
| (G3) 항등원 | `0 +_m a = a +_m 0 = a` | `0 ∈ ℤ_m` 이고 정수 덧셈에서 `0` 이 항등원 |
| (G4) 역원 | `a ≠ 0 ⟹ a +_m (m − a) = (m − a) +_m a = 0` <br> `0 +_m 0 = 0` | `(m − a) ∈ ℤ_m` (왜냐하면 `0 < m − a < m`) |
| (G5) 교환성 | `a +_m b = b +_m a` | 정수 덧셈의 교환성을 mod 산술로 옮긴 것 |

다섯 줄이 모두 *참* 이므로, `(ℤ_m, +_m)` 은 *교환군*이다.

특히 (G4) 의 역원을 명시해 두자. ℤ_m 에서 `a` 의 *덧셈에 대한 역원*은 다음과 같이 주어진다.
...
가령 ℤ_7 에서 `3` 의 덧셈 역원은 `7 − 3 = 4` 이다. 검산 — `3 +_7 4 = 7 mod 7 = 0`. 정확히 항등원이 나온다.

### 1.12.4.5 ℤ_m 의 *곱셈*은 군이 아니다

방금 ℤ_m 의 *덧셈* 이 교환군임을 보았다. 그렇다면 ℤ_m 의 *곱셈* 은 어떨까? 다음 표로 확인한다.

| 군 공리 | ℤ_m 의 곱셈 `·_m` 에 대해 | 만족 여부 |
|---|---|---|
| (G1) 폐쇄성 | `a, b ∈ ℤ_m ⟹ (a ·_m b) ∈ ℤ_m` | 만족 |
| (G2) 결합성 | `(a ·_m b) ·_m c = a ·_m (b ·_m c)` | 만족 |
| (G3) 항등원 | `1 ·_m a = a ·_m 1 = a` | 만족 |
| (G4) 역원 | 모든 `a` 에 대해 `a⁻¹` 존재? | *깨짐* |

(G4) 의 역원 공리가 깨진다. 가령 ℤ_6 에서 `2` 의 곱셈에 대한 역원은 *없다*. 모든 `b ∈ {0, 1, 2, 3, 4, 5}` 에 대해 `2 ·_6 b ∈ {0, 2, 4}` 이고, 어떤 `b` 도 `1` 을 만들지 못한다.

따라서 `(ℤ_m, ·_m)` 은 일반적으로 *군이 아니다*. ‘0 을 제외하면 군이 되지 않을까?' 라는 질문도 자연스러우나, ℤ_6 의 예에서 보았듯 `2` 자체에 역원이 없으므로 ‘0 을 빼낸 ℤ_m \ {0}' 위의 곱셈도 일반적으로 군이 되지 않는다.

곱셈에 대해서도 군이 되려면 *추가 조건*이 필요하다. 그 조건은 다음 한 줄로 요약된다.

> ℤ_m \ {0} 이 곱셈에 대해 군을 이룬다 ⟺ `m` 이 *소수*(prime)이다.

이 사실은 §4.4 (베주의 정리에서 도출되는 모듈러 곱셈 역원의 존재 조건) 의 직접 따름정리이다. ℤ_p (`p` 소수) 가 ‘0 이 아닌 모든 원소가 곱셈 역원을 가진다' 는 사실이 *체*(field) 라는 추상 구조로 가는 출발점이며, 이는 RSA를 비롯한 모든 공개키 암호의 수학적 토대이다.

### 1.12.4.6 Lean 4 검증 — ℤ_m이 교환군임을 형식적으로 확인

Mathlib 의 `ZMod m` 은 우리의 ℤ_m 에 대응한다. Mathlib은 `ZMod m` 이 `AddCommGroup` (덧셈에 대한 교환군) 의 인스턴스를 자동으로 갖춤을 *이미 증명해 두었다*.

```lean
import Mathlib

-- ZMod m 은 임의의 m 에 대해 덧셈 교환군이다.
example (m : ℕ) [NeZero m] : AddCommGroup (ZMod m) := inferInstance

-- 덧셈 역원 검증 — ZMod 7 에서 3 의 역원은 4 (= 7 - 3) 이다.
example : (3 : ZMod 7) + 4 = 0 := by rfl

-- 항등원 검증 — 0 이 덧셈 항등원이다.
example (a : ZMod 7) : (0 : ZMod 7) + a = a := by rw [zero_add]

-- 결합성 검증 — 정수 결합성이 그대로 옮겨 온다.
example (a b c : ZMod 7) : (a + b) + c = a + (b + c) := by rw [add_assoc]

-- 교환성 검증.
example (a b : ZMod 7) : a + b = b + a := by rw [add_comm]
```

`inferInstance` 는 ‘Mathlib 라이브러리에서 *교환군* 인스턴스를 자동으로 찾아 와라' 는 명령이며, 한 줄로 끝난다. 이는 우리가 위 §1.12.4.4 에서 손으로 검증한 다섯 줄을 Lean의 *typeclass 시스템* 이 라이브러리 수준에서 이미 박제해 두었음을 의미한다.

`(3 : ZMod 7) + 4 = 0` 이 `rfl` 한 줄로 닫힌다는 사실에 주목한다. `rfl` 은 ‘양변이 *정의에 의해* 같다' 는 전술이다. ZMod 의 덧셈이 ‘정수 덧셈 후 mod' 로 *계산 가능하게* 정의되어 있어, 컴파일러가 자동으로 양변이 같음을 확인한다.

### 1.12.4.7 군 구조의 의미 — 왜 이름을 붙이는가

여기서 자연스러운 질문이 떠오른다. *‘여섯 가지 성질을 나열하면 충분한데, 왜 그 중 네 가지에 굳이 군이라는 이름을 붙이는가?'*

답은 **추상화의 이득**(benefit of abstraction)에 있다. 한 번 ‘이 구조는 군이다' 라고 인정되면, 군에 대해 *이미 증명된 모든 정리* 가 이 구조에도 자동으로 적용된다. 예를 들어 다음 사실은 *임의의* 군에서 자동으로 성립한다.

- *항등원의 유일성* — 군의 항등원 `e` 는 단 하나뿐이다.
- *역원의 유일성* — 각 원소의 역원은 단 하나뿐이다.
- *소거 법칙*(cancellation law) — `a * b = a * c ⟹ b = c` 그리고 `b * a = c * a ⟹ b = c`.
- *역원의 역원* — `(a⁻¹)⁻¹ = a`.

위 네 사실은 ℤ_m 에서 따로 증명할 필요가 없다. ℤ_m 이 군임을 한 번만 보이면, 군의 일반 정리가 모두 *공짜로* 따라온다. 이것이 19세기 수학자들이 군이라는 이름을 정의한 이유이고, Mathlib 같은 형식 라이브러리가 ‘추상 구조 → 인스턴스 등록' 패턴을 따르는 이유이며, 이 강의에서 ℤ_m 의 여섯 성질을 본 직후에 바로 군이라는 단어를 도입하는 이유이다.

### 1.12.4.8 최신 AI·암호학과의 연결 — 왜 학부생도 군을 알아야 하는가

군 구조는 현대 컴퓨터과학의 도처에 숨어 있다. 몇 가지만 짚는다.

- **타원곡선 암호**(elliptic curve cryptography) — 타원곡선의 점들이 이루는 군. 같은 RSA 수준의 보안을 *훨씬 짧은 키* 로 달성. 비트코인, 이더리움 서명에 사용.
- **격자 기반 양자 후 암호**(lattice-based post-quantum cryptography) — `(ℤ_q)ⁿ` 위의 군. 양자 컴퓨터에 깨지지 않는 차세대 암호.
- **신경망의 등변 표현**(equivariant neural networks) — 입력에 대한 변환이 *군* 을 이룰 때, 신경망이 자동으로 그 군에 대해 ‘공평하게' 작동하도록 설계. 분자구조 인식 등 과학·의료 AI 에서 폭발적으로 성장 중.
- **체인 룰을 가진 미분 가능 프로그래밍** — 자동 미분이 함수의 합성에 대해 군에 가까운 구조를 사용한다. PyTorch·JAX의 backward pass가 이 위에 서 있다.

ℤ_m 이라는 한 학기 분량의 작은 예시를 통해 우리는 이 모든 분야의 *공통 언어*를 처음 만난다.

---

## 1.12.5 군과 고리(ring)의 첫 등장 — 종합

(*기존 §1.12.4 가 이 자리로 옮겨진다. 본 절은 위 §1.12.4 의 군 정의를 기반으로 한 단계 더 추상화한 ‘고리' 개념을 도입한다.*)

§1.12.4 에서 ℤ_m 의 *덧셈 부분* 만 따로 떼면 교환군이 됨을 확인했다. 이제 *덧셈과 곱셈을 함께* 보면 어떤 구조가 되는가?

> **정의 (고리)** — 집합 `R` 과 두 이항연산 `+, ·` 의 삼중쌍 `(R, +, ·)` 가 다음을 모두 만족할 때, **고리**(ring)라 한다.
>
> **(R1)** `(R, +)` 는 *교환군*이다. (덧셈에 대한 (G1)~(G5) 모두 성립.)
> **(R2)** 곱셈 `·` 은 *결합적*이다. (G2 의 곱셈 버전.)
> **(R3)** 곱셈에 *항등원* `1 ∈ R` 이 존재한다. (G3 의 곱셈 버전.)
> **(R4)** *분배성* 이 성립한다. `a · (b + c) = (a · b) + (a · c)` 그리고 `(a + b) · c = (a · c) + (b · c)`.
>
> 추가로 **(R5)** 곱셈도 교환적이면 **교환고리**(commutative ring)라 한다.

고리는 군보다 *한 단계 더 풍부* 한 구조이다. 군이 ‘되돌릴 수 있는 한 가지 연산' 이라면, 고리는 ‘되돌릴 수 있는 덧셈과, 분배되는 곱셈을 함께 가진' 구조이다.

이 정의에서 결정적인 점은 **곱셈에 대한 역원은 요구하지 않는다** 는 것이다. 그래서 ℤ 도 고리이고 (`(ℤ, +)` 는 군이지만 `(ℤ \ {0}, ×)` 는 군이 아님에도) ℤ_m 도 고리이다. 군은 너무 좁고, 고리는 군 위에 곱셈을 하나 더 얹은 ‘느슨한 일반화' 이다.

§1.12.3 의 여섯 성질을 다시 보자. 이 여섯 성질이 정확히 *교환고리의 공리*이다.

- (R1) ⟸ §1.12.3 의 ‘폐쇄성·결합성·교환성·항등원·덧셈 역원' (= (G1)~(G5))
- (R2) ⟸ §1.12.3 의 ‘결합성' 의 곱셈 버전 (정리 5)
- (R3) ⟸ §1.12.3 의 ‘항등원' 의 곱셈 버전 (`1` 이 곱의 항등원)
- (R4) ⟸ §1.12.3 의 ‘분배성'
- (R5) ⟸ 정리 5 의 따름정리 (정수 곱셈의 교환성)

여섯 성질이 정확히 다섯 공리에 대응하므로 결론은 다음과 같다.

> **결론** — `(ℤ_m, +_m, ·_m)` 은 *교환고리* 이다. 정수 `(ℤ, +, ·)` 도 교환고리이다. 즉 **ℤ_m 은 ℤ 의 유한 형제** 이다.

Mathlib 검증:

```lean
-- ZMod m 은 임의의 m 에 대해 교환고리이다.
example (m : ℕ) [NeZero m] : CommRing (ZMod m) := inferInstance
```

한 줄로 닫힌다. 이 한 줄 뒤에 RSA·타원곡선·격자 암호의 *수학 전체* 가 서 있다.

---

## 자기 점검 (군과 고리)

본 절을 마쳤다면 다음 일곱 가지 질문에 한 호흡으로 답해 보아라.

1. 군의 네 공리는 무엇인가? → ____
2. 교환군이 일반 군과 다른 *추가* 공리는? → ____
3. `(ℕ, +)` 이 군이 *아닌* 이유는? → ____
4. `(ℤ_7, +_7)` 에서 `5` 의 덧셈 역원은? → ____
5. `(ℤ_6, ·_6)` 에서 `2` 의 곱셈 역원은? → ____
6. 군과 고리의 가장 큰 차이는? → ____
7. ℤ_m \ {0} 의 곱셈이 군이 되는 *필요충분조건*은? → ____

<details>
<summary>정답</summary>

1. (G1) 폐쇄성, (G2) 결합성, (G3) 항등원, (G4) 역원.
2. (G5) 교환성 `a * b = b * a`.
3. `2 + (-2) = 0` 인데 `-2` 가 자연수가 아니어서 (G4) 역원이 깨진다.
4. `7 − 5 = 2`. 검산 `5 +_7 2 = 7 mod 7 = 0`.
5. *없다*. ℤ_6 에서 `2 ·_6 b ∈ {0, 2, 4}` 이라 `1` 을 못 만든다.
6. 군은 *연산 하나*, 고리는 *연산 둘 (+ 분배성)*. 또한 군은 (가환·역원 가짐) 인 반면, 고리는 곱셈에 대해 역원을 요구하지 않는다.
7. `m` 이 *소수* 일 때만. 이는 §4.4 베주의 정리의 직접 따름정리이다.

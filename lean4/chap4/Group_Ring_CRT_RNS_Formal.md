...
**(1) 잘 정의됨**. $[x]_M = [y]_M$이면 $M \mid (x - y)$이다. 각 $i$에 대해 $m_i \mid M$이므로 $m_i \mid (x - y)$이다. 즉, $[x]_{m_i} = [y]_{m_i}$이다. 따라서 $\phi([x]_M) = \phi([y]_M)$이다.
...
\phi([x+y]_M)
=
([x+y]_{m_1}, \ldots, [x+y]_{m_k})
$$

$$
=
([x]_{m_1} + [y]_{m_1}, \ldots, [x]_{m_k} + [y]_{m_k})
=
\phi([x]_M) + \phi([y]_M).
$$
...
\phi([1]_M)
=
([1]_{m_1}, [1]_{m_2}, \ldots, [1]_{m_k})
=
1_{R_1 \times \cdots \times R_k}.
$$

**(3) 단사**. $\phi([x]_M) = \phi([y]_M)$이면 모든 $i$에 대해 $[x]_{m_i} = [y]_{m_i}$이다. 즉, $m_i \mid (x - y)$이다. 보조정리 2.5에 의해 $M \mid (x - y)$이다. 따라서 $[x]_M = [y]_M$이다.

**(4) 전사**. 임의의 튜플 $([a_1], [a_2], \ldots, [a_k])$가 주어졌다고 하자. 정리 2.4 (c)에 의해

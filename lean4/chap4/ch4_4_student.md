# 이산수학 4.4절 합동 풀기 · 학생 안내노트

> **수업 운영 안내** — 본 안내노트는 강의 슬라이드(180장, 5부 구성)에 1:1 대응한다. 빈칸은 강의 중에 채우는 것을 원칙으로 하며, 학습자가 사전 학습 시 채우려고 시도해도 무방하다. 정답은 각 절 끝 `<details>` 토글로 제공한다. 모든 **Lean 4** 코드의 빈칸은 `____` 로 표시되며, `sorry` 는 어떤 경우에도 사용하지 않는다.
...
이 성립한다는 뜻이다. 즉, **a − b**(a − b)가 m의 배수이다.
